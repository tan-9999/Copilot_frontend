...
import { sendPrompt, validateDirectory, checkBackendHealth } from './services/api'
...
function App() {
  ...
  const handleDirectoryChange = async () => {
    ...
    try {
      ...
      setError(`Failed to validate directory: ${error.message}`)
    } finally {
      setIsTyping(false)
    }
  }
      ...
      <div className="bg-slate-800/30 backdrop-blur-sm px-6 py-3 border-b border-white/10">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-2">
            ...
            <span className="text-sm font-medium text-slate-200">Working Directory:</span>
          </div>
          <button 
